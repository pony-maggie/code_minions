...
def _delivery_guidance_context(ticket: dict[str, Any]) -> str:
    ...
    if _looks_like_turn_based_board_game(ticket):
        lines.append(
            ...
            "from the target line so black cannot complete five first. The black filler stones must not "
            "share one row, one column, or one diagonal. For example, a safe 10-move white vertical win "
            "uses black fillers `(10,10)`, `(11,12)`, `(12,14)`, `(13,11)`, `(14,13)` and white target "
            "cells `(0,5)`, `(1,5)`, `(2,5)`, `(3,5)`, `(4,5)`. A safe 10-move white diagonal win uses "
            "the same black fillers and white target cells `(0,0)`, `(1,1)`, `(2,2)`, `(3,3)`, `(4,4)`. "
            "A valid 9-click black-win sequence uses black target cells on turns 1/3/5/7/9 and white "
