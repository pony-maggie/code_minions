...
def _delivery_guidance_context(ticket: dict[str, Any]) -> str:
    ...
    if _looks_like_turn_based_board_game(ticket):
        lines.append(
            ...
            "from the target line so black cannot complete five first. A valid 9-click black-win sequence "
            "uses black target cells on turns 1/3/5/7/9 and white filler cells far away; a valid 10-click "
            "white-win sequence uses white target cells on turns 2/4/6/8/10 with black filler cells far "
            "away. If you need to test five same-color stones without turn alternation, test a pure "
