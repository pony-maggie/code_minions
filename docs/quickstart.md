...
stack-specific wrapper such as `react-vite-prd-to-pr`, `python-cli-prd-to-pr`,
or `python-web-prd-to-pr` when possible. Use the generic `prd-to-pr` only for
custom stacks or PRDs with a complete delivery contract. Then make sure all of
these are in place:
...
- `python-web` — Python FastAPI-style web API/service with pytest.
...
Stack-specific PRD-to-local-commits presets pin the delivery stack up front.
The Python web preset also uses a dedicated FastAPI planner that keeps small API
services in one canonical app task:
...
code-minions run python-web-prd-to-commit --input prd=./my-prd.md
```
